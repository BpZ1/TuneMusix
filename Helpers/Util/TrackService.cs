...
namespace TuneMusix.Helpers
{
    public static class TrackService
    {
        ...
        public static bool Contains( Track t, string s, bool caseSensitive )
        {
            if ( caseSensitive )
            {
                if ( t.Album.Value.Contains( s ) )
                {
                    return true;
                }

                if ( t.Interpret.Value.Contains( s ) )
                {
                    return true;
                }

                if ( t.Title.Value.Contains( s ) )
                {
                    return true;
                }
            ...
            {
                if ( t.Album.Value.ToLower().Contains( s.ToLower() ) )
                {
                    return true;
                }

                if ( t.Interpret.Value.ToLower().Contains( s.ToLower() ) )
                {
                    return true;
                }

                if ( t.Title.Value.ToLower().Contains( s.ToLower() ) )
                {
                    return true;
                }
